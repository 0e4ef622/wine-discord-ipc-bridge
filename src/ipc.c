...
PRIVATE DWORD WINAPI winwrite_thread(LPVOID lpvParam);
PRIVATE int iListenPipe();

int iServerMain(BOOL bStandaloneArg)
{
...
PRIVATE int iListenPipe()
{
	...
	for (int p = 0; p < sizeof(paths) / sizeof(paths[0]) && !connected; p++)
	{
		for (int pipeNum = 0; pipeNum < 10 && !connected; ++pipeNum)
		{

			StringCchPrintf(addr.sun_path, sizeof(addr.sun_path), paths[p], temp_path, pipeNum);
			INFO("Attempting to connect to %s\n", addr.sun_path);

			connected = !(l_connect(sock_fd, (struct sockaddr*) &addr, sizeof(addr)) < 0);
			if (!connected)
